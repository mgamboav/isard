...
class GrpcActions(object):
    ...
    def engine_info(self):
        ...
        try:
            if self.manager.t_background is not None:
                try:
                    self.manager.t_background.is_alive()
                except AttributeError:
                    d_engine['background_is_alive'] = False
                    return d_engine

                if self.manager.t_background.is_alive():
                    d_engine['background_is_alive'] = True
                    d_engine['event_thread_is_alive'] = self.manager.t_events.is_alive() if self.manager.t_events is not None else False
                    d_engine['broom_thread_is_alive'] = self.manager.t_broom.is_alive() if self.manager.t_broom is not None else False
                    d_engine['download_changes_thread_is_alive'] = self.manager.t_downloads_changes.is_alive() \
                                                                        if self.manager.t_downloads_changes is not None else False

                    d_engine['changes_hyps_thread_is_alive'] = self.manager.t_changes_hyps.is_alive() \
                                                                        if self.manager.t_changes_hyps is not None else False
                    d_engine[
                        'changes_domains_thread_is_alive'] = self.manager.t_changes_domains.is_alive() if self.manager.t_changes_domains is not None else False
                    d_engine['working_threads'] = list(self.manager.t_workers.keys())
                    d_engine['status_threads'] = list(self.manager.t_status.keys())
                    d_engine['disk_operations_threads'] = list(self.manager.t_disk_operations.keys())
                    d_engine['long_operations_threads'] = list(self.manager.t_long_operations.keys())

                    d_engine['alive_threads'] = dict()
                    d_engine['alive_threads']['working_threads'] = {name:t.is_alive() for name,t in self.manager.t_workers.items()}
                    d_engine['alive_threads']['status_threads'] = {name:t.is_alive() for name,t in self.manager.t_status.items()}
                    d_engine['alive_threads']['disk_operations_threads'] = {name:t.is_alive() for name,t in self.manager.t_disk_operations.items()}
                    d_engine['alive_threads']['long_operations_threads'] = {name:t.is_alive() for name,t in self.manager.t_long_operations.items()}

                    d_engine['queue_size_working_threads'] = {k: q.qsize() for k, q in self.manager.q.workers.items()}
                    d_engine['queue_disk_operations_threads'] = {k: q.qsize() for k, q in self.manager.q_disk_operations.items()}
                else:
