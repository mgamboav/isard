...
class isard_lib():
    ...
    def _get_disk_path(self, user, parsed_name):
        dir_path = user.category+'/'+user.group+'/'+user.id
    ...
    def desktop_get(self,user,id):
        with app.app_context():
            data = list(r.table('domains').get_all(user.id, index='user').filter({'id':id,'kind':'desktop'}).without('xml','history_domain').run(db.conn))
        if len(data) == 0:
            return False
        return data[0]
    ...
    def desktop_list(self,user):
        with app.app_context():
            data = list(r.table('domains').get_all(user.id, index='user').filter({'kind':'desktop'}).without('xml','history_domain').run(db.conn))
        if len(data) == 0:
            return False
        return data
    ...
    def desktop_add(self,user,from_template_id,data):
        ...
        ''' Allowed template?? '''
        templates = self.get_allowed_data(user,'domains')
        template = [t for t in templates if t['id'] == from_template_id]
        if len(template) == 0:
            return False
    ...
    def desktop_start(self,user,id):
        if not self._quota_check(user,'desktop','start'): return False
        with app.app_context():
            domain = list(r.table('domains').get_all(user.id, index='user').filter({'id':id,'kind':'desktop'}).pluck('status').run(db.conn))
        if len(domain) == 0:
            return False
    ...
    def template_list(self,user):
        with app.app_context():
            ...
            data_base = list(r.table('domains').get_all(user.id, index='user').filter({'kind':'base'}).without('xml','history_domain').run(db.conn))
            data = data_tmpl + data_base
        if len(data) == 0:
            return False
        return data
    ...
    def media_get(self,user,id):
        with app.app_context():
            data = list(r.table('media').get_all(user.id, index='user').filter({'id':id}).run(db.conn))
        if len(data) == 0:
            return False
        return data[0]
    ...
    def media_list(self,user):
        with app.app_context():
            data = list(r.table('media').get_all(user.id, index='user').run(db.conn))
        if len(data) == 0:
            return False
        return data   
    ...
    def user_update_password(self,user,id,passwd):
        if user.id != id: return False
        pw=Password()
        with app.app_context():
            return self._rdbcheck(r.table('users').get(id).update({'password':pw.encrypt(passwd)}).run(db.conn),'replaced')        
    
    def user_allowed_hardware(self,user):
         return {'nets':         self.get_allowed_data(user,'interfaces'),
                'graphics':     self.get_allowed_data(user,'graphics'),
                'videos':       self.get_allowed_data(user,'videos'),
                'boots':        self.get_allowed_data(user,'boots'),
                'hypervisors_pools': self.get_allowed_data(user,'hypervisors_pools'),
                'forced_hyps':  self.get_allowed_data(user,'hypervisors'),
                'user': user.quota}
    ...
    ''' FORMS DATA '''
    def get_allowed_data(self,user,table,pluck=False):
        if table == 'domains':
            if pluck is False: pluck = {'id','name','allowed','kind','group','icon','user','description'}
            with app.app_context():
                ud=r.table('users').get(user.id).run(db.conn)
                data1 = r.table('domains').get_all('base', index='kind').order_by('name').pluck(pluck).run(db.conn)
                data2 = r.table('domains').filter(r.row['kind'].match("template")).order_by('name').pluck(pluck).run(db.conn)
            data = data1+data2
        elif table in ['interfaces','graphics','video','boots','hypervisors_pools']:
            if pluck is False: pluck = {'id','name','allowed','user','description'}
            data = r.table(table).order_by('name').pluck(pluck).run(db.conn)
        else:
            return False
        return self.is_allowed(user,data)

    def get_allowed_id(self,user,table,id,pluck=False):
        try:
            if table == 'domains' and pluck is False: 
                pluck = {'id','name','allowed','kind','group','icon','user','description'}
            else:
                pluck = {'id','name','allowed','user','description'}
            data = [r.table(table).get(id).pluck(pluck).run(db.conn)]
            filtered = self.is_allowed(user, data)
    ...
    def is_allowed(self,user,data):
        ...
        for d in data:
            ...
            # ~ if user['role']=='admin': 
                # ~ alloweds.append(d)
                # ~ continue
            ''' This try is to get quicker with domains, but will fail with others '''
            try:
                if d['user'] == user.id:
                    alloweds.append(d)
                    continue
    ...
    def get_quotas(self,user):
        with app.app_context():
            ...
            try:
                qpdesktops=desktops*100/user['quota']['domains']['desktops']
            except Exception as e:
                qpdesktops=100
            try:
                qpup=desktopsup*100/user['quota']['domains']['running']
            except Exception as e:
                qpup=100
            try:
                qptemplates=templates*100/user['quota']['domains']['templates']
            except:
                qptemplates=100
            try:
                qpisos=isos*100/user['quota']['domains']['isos']
            except:
                qpisos=100
        return {'d':desktops,  'dq':user['quota']['domains']['desktops'],  'dqp':"%.2f" % round(qpdesktops,2),
                'r':desktopsup,'rq':user['quota']['domains']['running'],   'rqp':"%.2f" % round(qpup,2),
                't':templates, 'tq':user['quota']['domains']['templates'], 'tqp':"%.2f" % round(qptemplates,2),
                'i':isos,      'iq':user['quota']['domains']['isos'],      'iqp':"%.2f" % round(qpisos,2)}
        

                
    ''' HELPERS '''
    def unique_id(self,table,id):
        if r.table(table).get(id).run(db.conn) is not None: return False
        return True

    def has_domain_access(self,user,id):
        with app.app_context():
            domains = self.template_list(current_user.username)
            domain = [d for d in domains if d['id'] == id]
        if len(domain) == 0:
            return False
        return True
